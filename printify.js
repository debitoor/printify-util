	...
	return function (content, options, callback) {
		...
		function doIt() {
			...
			$.each(['left', 'right', 'top', 'bottom'], function (i, key) {
				parts[key] = overload(options[key]);
			});
			...
			// yes $.width in try-catch because doIt may execute after docuemnt does not exists any more
			// doIt wrapper in setTimeout, cmon
			try {
			...
			var update = function () {
				...
				return until(Array.prototype.slice.apply($content.find('.page-break:not(.page-break-visited)')), function (el) {
					...
					if ($el.offset().top - contentOffset > nextBreak && $prev) {
						...
						var $top = $('<div class="page-top">' + (parts.top(nextPage) || '') + '</div>').appendTo($content);
						var $bottom = $('<div class="page-bottom">' + (parts.bottom(nextPage) || '') + '</div>').appendTo($content);
						var $left = $('<div class="page-left">' + (parts.left(nextPage) || '') + '</div>').appendTo($content);
						var $right = $('<div class="page-right">' + (parts.right(nextPage) || '') + '</div>').appendTo($content);
			...
			return callback && callback(null, {
					...
					render: function (callback) {
						return printify(content, options, callback);
					}
				});
		}
	};
